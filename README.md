...
- 在“问答补全”（原训练模式）中增加了 AI 润色 按钮。
- 用户输入草稿答案后，点击按钮即可调用大模型对答案进行专业化润色，提升回复质量。
- 普通用户权限扩展

- 问答补全 ：普通用户现在可以访问此功能并提交问答对，提交后状态为 pending （待审批），需管理员审核通过后才会生效。
- 问答记录 ：普通用户现在可以查看历史问答记录。
- 未知问题 ：普通用户可以查看未知问题并提交解答，同样需要经过管理员审批。
- 菜单与界面优化

- 审批中心 ：将“知识库审批”重命名为“审批中心”，并采用标签页设计，分别管理 文档审批 和 问答审批 。
- 未知问题 ：重命名菜单，并增加了 丢弃 功能，管理员可将无效问题标记为丢弃，不再显示。
- 问答补全 ：将“训练模式”重命名为“问答补全”，表意更准确。
